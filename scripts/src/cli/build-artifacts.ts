...
fs.writeFileSync(path.join(dataDir, 'points.json'), JSON.stringify(points));
...
fs.writeFileSync(path.join(dataDir, 'graph_media_relations.json'), JSON.stringify(rel));
fs.writeFileSync(path.join(dataDir, 'graph_media_staff.json'), JSON.stringify(graphMediaStaff));
fs.writeFileSync(path.join(dataDir, 'graph_people_collab.json'), JSON.stringify(collab));
...
const toChunks = (arr: any[], prefix: string, lookupFile: string) => {
  ...
  for (let i = 0; i < arr.length; i += 500) {
    ...
    fs.writeFileSync(path.join(metaDir, name), JSON.stringify(chunk, null, 2));
    for (const row of chunk) lookup[String(row.id)] = name;
  }
  fs.writeFileSync(path.join(lookupDir, lookupFile), JSON.stringify(lookup));
};

toChunks(media, 'media', 'media_to_meta_chunk.json');
toChunks(people, 'people', 'people_to_meta_chunk.json');
fs.writeFileSync(path.join(metaDir, 'characters_00000.json'), '[]');
...
fs.writeFileSync(path.join(indexDir, 'search.json'), JSON.stringify({ media: media.map((m) => ({ id: m.id, title: m.title })), people: people.map((p) => ({ id: p.id, name: p.name })) }));
fs.writeFileSync(path.join(indexDir, 'tag_to_media.json'), JSON.stringify(tagToMedia));
fs.writeFileSync(path.join(indexDir, 'role_to_people.json'), JSON.stringify(roleToPeople));
fs.writeFileSync(path.join(indexDir, 'tag_role_to_people.json'), JSON.stringify({}));
fs.writeFileSync(path.join(indexDir, 'year_buckets.json'), JSON.stringify(year));
fs.writeFileSync(path.join(dataDir, 'clusters.json'), JSON.stringify(Array.from({ length: 12 }, (_, id) => ({ cluster_id: id, label: `Cluster ${id}`, centroid: [Math.sin(id), Math.cos(id)] }))));
